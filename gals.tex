...
\begin{description}
\item[Notation:] Die Projektion, die gegeben einer Indexmenge $I$ die entsprechenden Elemente aus einem Tupel $X$ auswählt, ohne die Reihenfolge zu verändern wird mit
  \[ X |_I \]
  abgekürzt.
...
\begin{description}
\item[Notation:] Es kann ohne Beschränkung der Allgemeinheit davon ausgegangen werden, dass eine Ordnung auf den Automatennamen existiert, die bei der Erstellung von Eingabe- und Ausgabetupeln eingehalten wird.
...
Für eine Verbindung $((X,i),(Y,j))$ muss immer gelten, dass die Ausgabesymbole den Eingabesymbolen entsprechen, also $\Omega_i^X = \Sigma_j^Y$ gilt.
...
\begin{eqnarray*}
  ...
  (q,s,i,a) &\mapsto& \pi_O^a(q,s,\delta^a(\pi_I^a(q,s,i)))
\end{eqnarray*}
